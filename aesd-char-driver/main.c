                ...
                loff_t *f_pos)
{
    ...
    cmd = aesd_circular_buffer_find_entry_offset_for_fpos(aesd_device.cbuf, *f_pos, byte_rtn);
    if (!cmd) {
                ...
                loff_t *f_pos)
{
    ...
    if (!cmd_offset) {
        ...
        memset(cmd->buffptr + cmd_offset, 0, count);
    }

    if (mutex_lock_interruptible(&aesd_device.lock))
		return -ERESTARTSYS;
    
    if (copy_from_user(cmd->buffptr + cmd_offset, buf, count) != 0) {
    ...
    if (*(cmd->buffptr + (cmd_offset-1)) == '\n') {
        *(cmd->buffptr + cmd_offset) = '\0';
        cmd->size = cmd_offset;
        PDEBUG("From user (size = %ld): %s", cmd->size, cmd->buffptr);
        aesd_circular_buffer_add_entry(aesd_device.cbuf, cmd);
...
int aesd_init_module(void)
{
	...
	if (!aesd_device.cbuf) {
        printk(KERN_ERR "Can't allocate memory for circular v´buffer\n");
		return result;
	}
